...
import org.kontalk.konbot.util.ByteArrayInOutStream;
 ...
 */
public class PersonalKeyImporter implements PersonalKeyPack, Closeable {
        ...
        try {
            while ((entry = mKeyPack.getNextEntry()) != null) {
                ...
                if (PUBLIC_KEY_FILENAME.equals(entry.getName())) {
                    ...
                    publicKey = StreamUtils.readFully(mKeyPack, MAX_KEY_SIZE);
                }

                // PGP private key
                else if (PRIVATE_KEY_FILENAME.equals(entry.getName())) {
                    // I don't really know if this is good...
                    privateKey = StreamUtils.readFully(mKeyPack, MAX_KEY_SIZE);
                }

                // trusted keys
                else if (TRUSTED_KEYS_FILENAME.equals(entry.getName())) {
                    // I don't really know if this is good...
                    trustedKeys = StreamUtils.readFully(mKeyPack, MAX_KEY_SIZE);
                }

                // account info
                else if (ACCOUNT_INFO_FILENAME.equals(entry.getName())) {
                    // I don't really know if this is good...
                    accountInfo = StreamUtils.readFully(mKeyPack, MAX_KEY_SIZE);
                }
